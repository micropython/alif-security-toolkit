...
class KeyArmCertificateHeader(ArmCertificateHeader):
    ...
    def __init__(self, cert_version, hbk_id, logger=None):
        ...
        self._hbk_id = hbk_id
        self.logger = logger
        self._signed_content_size = (
            global_defines.HEADER_SIZE_IN_WORDS
...
class ContentArmCertificateHeader(ArmCertificateHeader):
    ...
    def __init__(
        ...
        self._cert_version = (cert_version[0] << 16) + cert_version[1]
        self.logger = logger
        sizeof_sw_records_field = global_defines.NONCE_SIZE_IN_WORDS + num_of_comps * (
            global_defines.HASH_ALGORITHM_SHA256_SIZE_IN_WORDS
...
class EnablerDebugArmCertificateHeader(ArmCertificateHeader):
    ...
    def __init__(self, cert_version, rma_mode, hkb_id, lcs, logger=None):
        ...
        self._flags = (
            hkb_id << global_defines.HBK_ID_FLAG_BIT_OFFSET
...
class KeyArmCertificateBody(ArmCertificateBody):
    def __init__(
        ...
        self._signer_rsa_public_key = cryptolayer.Common.get_n_and_np_from_keypair(
            signer_keypair_filename, signer_keypair_passphrase_filename, self.logger
        )
        self._hashed_pubkey_next_cert = (
            cryptolayer.Common.get_hashed_n_and_np_from_public_key(
                next_cert_pubkey_filename, self.logger
            )
        )
...
class ContentArmCertificateBody(ArmCertificateBody):
        ...
        for line in image_list:
            if not re.match(r"^#", line):  # ignore commented lines
                ...
                if mem_load_address == 0:
                    raise CertCreationError(
                        "Invalid load address in images-table: 0 is not allowed as an address"
                    )
                mem_load_address_byte_list = list(
                    mem_load_address.to_bytes(
                        global_defines.NUM_OF_BYTES_IN_ADDRESS, byteorder="big"
                    )
                )
                ...
                if (
                    self._load_verify_scheme == global_defines.VERIFY_IMAGE_IN_FLASH
                    ...
                    raise CertCreationError(
                        "invalid load address defined in images-table: "
        ...
        for item in parsed_images:
            if item["is_aes_code_enc_used"] == 1:
                ...
                if code_enc_id != global_defines.USE_AES_CE_ID_NONE:
                    ...
                    if (
                        len(str_key_data_bytes)
                        > global_defines.AES_DECRYPT_KEY_SIZE_IN_BYTES
                    ):
                        self.logger.warning(
                            "key size in aes-enc-key file is too big - truncating parameter"
                        )
                    ...
                    aes_iv = (
                        self._key_nonce
                        + bytes(item["mem_load_address_byte_list"])
                        + bytes(4)
                    )
                    # encrypt image
                    new_image_filename = (
                        item["image_filename"][:-4]
                        + global_defines.SW_COMP_FILE_NAME_POSTFIX
                    )
                    hash_of_image, actual_image_size = (
                        cryptolayer.Common.encrypt_file_with_aes_ctr(
                            item["image_filename"],
                            new_image_filename,
                            key_data_list,
                            aes_iv,
                    ...
                    raise CertCreationError(
                        "invalid aes-ce-id for image encryption flag in image of images-table - "
            ...
            self._sw_records.append(
                [
                    hash_of_image,
                    item["mem_load_address"],
                    item["image_max_size"],
                    item["is_aes_code_enc_used"],
                ]
            )
            self._none_signed_info.append(
                [item["flash_store_address"], actual_image_size]
            )
    ...
    def serialize_to_bytes(self):
        data = (
            ...
            + b"".join(
                [
                    struct.pack("<I", item) if type(item) is not bytes else item
                    for sublist in self._sw_records
                    for item in sublist
                ]
            )
        )
    ...
    def none_signed_info_serialized(self):
        return b"".join(
            [
                struct.pack("<I", item)
                for sublist in self._none_signed_info
                for item in sublist
            ]
        )
    ...
    def __init__(
        ...
        with open(self._soc_id_filename, "rb") as soc_id_holder_file:
            ...
            if len(self._soc_id) != global_defines.SOC_ID_SIZE_IN_BYTES:
                raise ValueError(
                    "Invalid SoC_ID size in input file " + self._soc_id_filename
                )
...
class ArmCertificateSignature:
    def __init__(
        ...
        self.logger = logger
        self._signature = cryptolayer.Common.rsa_sign(
            cert_data_to_sign,
            signer_keypair_filename,
            signer_keypair_passphrase_filename,
            self.logger,
        ...
        self._signature = self._signature[
            ::-1
        ]  # for some reason the original script reverses the signature
...
class KeyArmCertificate(ArmCertificate):
    def __init__(self, certificate_config, cert_version, logger=None):
        ...
        self._cert_header = KeyArmCertificateHeader(
            self._cert_version, self._cert_config.hbk_id, self.logger
        )
        self._cert_body = KeyArmCertificateBody(
            self._cert_config.nvcounter_val,
        ...
        self._certificate_data = (
            self._cert_header.serialize_to_bytes()
            + self._cert_body.serialize_to_bytes()
        )
        self._cert_signature = ArmCertificateSignature(
            self._certificate_data,
            self._cert_config.cert_keypair,
            self._cert_config.cert_keypair_pwd,
            self.logger,
...
class ContentArmCertificate(ArmCertificate):
    def __init__(self, certificate_config, cert_version, logger=None):
        ...
        self._cert_body = ContentArmCertificateBody(
            self._cert_config.nvcounter_val,
            self._cert_config.cert_keypair,
            self._cert_config.cert_keypair_pwd,
        ...
        self._cert_header = ContentArmCertificateHeader(
            self._cert_version,
            self._cert_config.aes_ce_id,
            self._cert_config.load_verify_scheme,
            self._cert_config.crypto_type,
...
class EnablerDebugArmCertificate(ArmCertificate):
    def __init__(self, certificate_config, cert_version, logger=None):
        ...
        self._cert_config = certificate_config
        self._cert_version = cert_version
        self._cert_header = EnablerDebugArmCertificateHeader(
            cert_version,
        ...
        self._cert_body = EnablerDebugArmCertificateBody(
            self._cert_config.cert_keypair,
            self._cert_config.cert_keypair_pwd,
...
class DeveloperDebugArmCertificate(ArmCertificate):
    def __init__(self, certificate_config, cert_version, logger=None):
        ...
        self._cert_config = certificate_config
        self._cert_version = cert_version
        self._cert_header = DeveloperDebugArmCertificateHeader(
            cert_version, self.logger
        )
        self._cert_body = DeveloperDebugArmCertificateBody(
            self._cert_config.cert_keypair,
            self._cert_config.cert_keypair_pwd,
            self._cert_config.debug_masks,
