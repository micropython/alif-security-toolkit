...
class RsaCrypto:
        ...
        if password_filename is not None:
            try:
                ...
                if logger is not None:
                    logger.warning(
                        "Could not open passphrase file %s" % password_filename
                    )
                raise ValueError("Could not open passphrase file: %s" % e)
        else:
            passphrase = getpass.getpass(
                prompt="Enter passphrase for creating RSA key <<"
                + pem_filename
                + ">>: "
            ).encode("utf-8")
    ...
    @staticmethod
    def load_rsa_pem_key(
        pem_filename, is_key_private, keypair_passphrase_filename=None, logger=None
    ):
        """
        ...
        if is_key_private is True:
            if len(keypair_passphrase_filename) > 0:
                try:
                    ...
                    if logger is not None:
                        logger.warning(
                            "Could not open keypair passphrase file %s"
                            % keypair_passphrase_filename
                        )
                    raise RsaKeyLoadingError(
                        "Could not open keypair passphrase file: %s" % e
                    )
            else:
                passphrase = getpass.getpass(
                    prompt="Enter passphrase for RSA private key: "
                ).encode("utf-8")
        # loading public/private key
        try:
            if is_key_private is True:  # private key case
                key = load_pem_private_key(
                    bytes(pem_data, encoding="utf-8"),
                    passphrase,
                ...
                if isinstance(key, rsa.RSAPrivateKey) is False:
                    if logger is not None:
                        logger.warning(
                            "Could not parse key as an RSA Private key from file %s"
                            % pem_filename
                        )
                    raise RsaKeyLoadingError(
                        "Could not parse PEM file as an RSAPrivateKey"
                    )
            else:  # public key case
                key = load_pem_public_key(
                    bytes(pem_data, encoding="utf-8"), backend=default_backend()
                )
                # check return value
                if isinstance(key, rsa.RSAPublicKey) is False:
                    if logger is not None:
                        logger.warning(
                            "Could not parse key as an RSA Public key from file %s"
                            % pem_filename
                        )
                    raise RsaKeyLoadingError(
                        "Could not parse PEM file as an RSAPublicKey"
                    )
    ...
    def concatenate_n_and_np(n_as_modulus, param_np, logger=None):
        ...
        n_in_bytes = n_as_modulus.to_bytes(
            global_defines.SB_CERT_RSA_KEY_SIZE_IN_BYTES, "big"
        )
    ...
    @staticmethod
    def verify_signature_pkcs_v21(
        signature, prehashed_message, public_key, logger=None
    ):
        """
...
class Common:
    ...
    def get_hashed_n_and_np_from_public_key(public_key_filename, logger=None):
        n_and_np_bytes = Common.get_n_and_np_from_public_key(
            public_key_filename, logger
        )
        digest = HashCrypto.calculate_sha256_hash(n_and_np_bytes)
        return digest

    @staticmethod
    def get_n_and_np_from_keypair(
        keypair_filename, keypair_passphrase_file, logger=None
    ):
        key = RsaCrypto.load_rsa_pem_key(
            keypair_filename, True, keypair_passphrase_file, logger=logger
        )
    ...
    def rsa_sign(data_to_sign, keypair_filename, keypair_passphrase_file, logger=None):
        ...
        RsaCrypto.verify_signature_pkcs_v21(
            signature, hashed_data_to_sign, key.public_key(), logger
        )
        return signature

    @staticmethod
    def rsa_verify_with_pubkey_params(
        rsa_pubkey_n, signed_data, signature, logger=None
    ):
        modulus_in_bytes = rsa_pubkey_n[
            0 : global_defines.PUBKEY_SIZE_BYTES
        ]  # chop of np is input contains it
        ...
        RsaCrypto.verify_signature_pkcs_v21(
            signature, hashed_data_to_sign, public_key, logger
        )

    @staticmethod
    def decrypt_data_with_rsa_keypair(
        keypair_filename, keypair_passphrase_file, ciphertext, logger=None
    ):
        private_key = RsaCrypto.load_rsa_pem_key(
            keypair_filename, True, keypair_passphrase_file, logger=logger
        )
    ...
    @staticmethod
    def encrypt_file_with_aes_ctr(
        input_filename, output_filename, key_int_array, aes_iv, hash_type, logger=None
    ):
        """
    ...
    def derive_aes_key(enc_password_filename, logger=None):
        ...
        if len(enc_password_filename) > 0:
            try:
                ...
                if logger is not None:
                    logger.warning(
                        "Could not open passphrase file %s" % enc_password_filename
                    )
                raise ValueError("Could not open passphrase file: %s" % e)
        else:
            passphrase = getpass.getpass(
                prompt="Enter passphrase for encrypted key: "
            ).encode("utf-8")
    ...
    @staticmethod
    def encrypt_asset_with_aes_cbc(
        input_filename, output_filename, enc_password_filename, logger=None
    ):
        ...
        # encrypt
        encrypted_data = AesCrypto.encrypt_aes_cbc(
            padded_unencrypted_data, aes_key, aes_iv
        )
