...
class ContentCertificateConfig:
    ...
    def load_verify_scheme(self, value):
        if isinstance(value, int) is False:
            ...
            raise ValueError(
                "invalid input value for config parameter load_verify_scheme"
            )
        else:
    ...
    def _parse_config(self):
        ...
        if not self._parser.has_section(self.section_name):
            if self._logger is not None:
                self._logger.error(
                    "section [" + self.section_name + "] wasn't found in cfg file"
                )
        ...
        if self._parser.has_option(
            self.section_name, "cert-keypair-pwd"
        ):  # used for testing
            self.cert_keypair_pwd = self._parser.get(
                self.section_name, "cert-keypair-pwd"
            )
        else:
        ...
        if not self._parser.has_option(self.section_name, "load-verify-scheme"):
            ...
            self.load_verify_scheme = self._parser.getint(
                self.section_name, "load-verify-scheme"
            )
        ...
        if self.aes_ce_id != global_defines.USE_AES_CE_ID_NONE and (
            self.load_verify_scheme == global_defines.VERIFY_IMAGE_IN_FLASH
            or self.load_verify_scheme == global_defines.LOADING_ONLY_IMAGE
        ):
            if self._logger is not None:
                self._logger.error(
                    "invalid parameter combination for aes-ce-id and load-verify-scheme"
                )
            raise ConfigParsingError
