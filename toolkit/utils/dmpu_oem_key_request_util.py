...
class ArgumentParser:
    def __init__(self):
        ...
        self.parser = optparse.OptionParser(
            usage="usage: %prog cfg_file [log_filename]"
        )
...
class OemKeyRequestConfig:
    ...
    def oem_main_keypair(self, value):
        if isinstance(value, str) is False:
            ...
            raise ValueError(
                "Config parameter oem-main-keypair cannot be an empty string!"
            )
        else:
    ...
    def oem_enc_pubkey(self, value):
        if isinstance(value, str) is False:
            ...
            raise ValueError(
                "Config parameter oem-enc-pubkey cannot be an empty string!"
            )
        else:
...
class OemKeyRequestConfigParser:
    ...
    def parse_config(self):
        ...
        if not self.config.has_section(self._config_holder.section_name):
            self.logger.warning(
                "section ["
                + self._config_holder.section_name
                + "] wasn't found in cfg file"
            )
            return False

        if not self.config.has_option(
            self._config_holder.section_name, "oem-main-keypair"
        ):
            self.logger.warning("oem-main-keypair not found")
            return False
        else:
            self._config_holder.oem_main_keypair = self.config.get(
                self._config_holder.section_name, "oem-main-keypair"
            )

        if self.config.has_option(self._config_holder.section_name, "oem-main-keypwd"):
            self._config_holder.oem_main_keypwd = self.config.get(
                self._config_holder.section_name, "oem-main-keypwd"
            )

        if not self.config.has_option(
            self._config_holder.section_name, "oem-enc-pubkey"
        ):
            self.logger.warning("oem-enc-pubkey not found")
            return False
        else:
            self._config_holder.oem_enc_pubkey = self.config.get(
                self._config_holder.section_name, "oem-enc-pubkey"
            )
        ...
        if not self.config.has_option(self._config_holder.section_name, "oem-cert-pkg"):
            ...
            self._config_holder.oem_cert_pkg = self.config.get(
                self._config_holder.section_name, "oem-cert-pkg"
            )
...
class KeyRequestCertificateCreator:
    ...
    def __init__(self, logger, key_request_cfg):
    ...
    def create_certificate(self):
        ...
        certificate_length = self.DMPU_CERT_HEADER_SIZE_IN_BYTES + 2 * (
            self.PUBKEY_SIZE_BYTES + self.NP_SIZE_IN_BYTES
        )
        certificate_header = (
            struct.pack("<I", self.DMPU_OEM_KEY_REQ_TOKEN)
            + struct.pack("<I", self.DMPU_OEM_KEY_REQ_VERSION)
            + struct.pack("<I", certificate_length)
        )

        # get the enabler certificate public key + Np
        oem_rsa_public_key_params = cryptolayer.Common.get_n_and_np_from_keypair(
            self.config.oem_main_keypair, self.config.oem_main_keypwd, self.logger
        )
        # get encrypted  public key  (from public key)
        oem_enc_pubkey_params = cryptolayer.Common.get_n_and_np_from_public_key(
            self.config.oem_enc_pubkey, self.logger
        )
        # Do RSA signature
        signed_data = (
            certificate_header + oem_rsa_public_key_params + oem_enc_pubkey_params
        )
        signature = cryptolayer.Common.rsa_sign(
            signed_data,
            self.config.oem_main_keypair,
...
def main(args):
    ...
    the_logger = util_logger.UtilLogger(
        the_argument_parser.log_filename, "OEM key request"
    )
    # get util configuration parameters
    config_parser = OemKeyRequestConfigParser(
        the_argument_parser.cfg_filename, the_logger
    )
    if config_parser.parse_config() is False:
        the_logger.critical("Config file parsing is not successful")
        sys.exit(-1)
    # create secure asset package
    asset_provisioner = KeyRequestCertificateCreator(
        the_logger, config_parser.get_config()
    )
    asset_provisioner.create_certificate()
    the_logger.info(
        "**** OEM key request generation has been completed successfully ****"
    )
