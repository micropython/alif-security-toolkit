...
def read_json_file(file, attributes, flags, cpus):
    ...
    for image in cfg:
        ...
        if len(image) == 0:
            print(
                "ERROR: image description can't be empty!",
                "Please correct it in configuration file",
            )
            sys.exit(EXIT_WITH_ERROR)
                    ...
                    if opt in ["loadAddress", "mramAddress"]:
                        ...
                        if opt == "loadAddress":
                            if userOptions[opt][0:3] == "0x8":
                                print(
                                    "ERROR: An MRAM address can not be used in loadAddress attribute"
                                )
                                sys.exit(EXIT_WITH_ERROR)
...
def cleanBuild(fileList):
    ...
    if confirmResponse.upper() == "Y" or confirmResponse.upper() == "YES":
        ...
        print(
            "\nThe build workspace was cleaned!",
            "(" + str(files) + " files were deleted)",
        )
    else:
    ...
    for sec in fwsections:
        if "COMPRESS" in sec["flags"]:
            ...
            if "ENCRYPT" in sec["flags"]:
                ...
                print(
                    "[ERROR] Please correct the configuration: "
                    + str(sec["flags"])
                    + " in image "
                    + sec["identifier"]
                )
