...
class AssetPackageConfig:
    ...
    def icv_enc_oem_key(self, value):
        if isinstance(value, str) is False:
            ...
            raise ValueError(
                "Config parameter icv-enc-oem-key cannot be an empty string!"
            )
        else:
    ...
    def oem_enc_keypair(self, value):
        if isinstance(value, str) is False:
            ...
            raise ValueError(
                "Config parameter oem-enc-keypair cannot be an empty string!"
            )
        else:
    ...
    def asset_filename(self, value):
        if isinstance(value, str) is False:
            ...
            raise ValueError(
                "Config parameter asset-filename cannot be an empty string!"
            )
        else:
...
class AssetPackageConfigParser:
    ...
    def parse_config(self):
        ...
        if not self.config.has_option(self._config_holder.section_name, "asset-type"):
            ...
            self._config_holder.asset_type = self.config.get(
                self._config_holder.section_name, "asset-type"
            )

        if not self.config.has_option(
            self._config_holder.section_name, "icv-enc-oem-key"
        ):
            self.logger.warning("icv-enc-oem-key not found")
            return False
        else:
            self._config_holder.icv_enc_oem_key = self.config.get(
                self._config_holder.section_name, "icv-enc-oem-key"
            )

        if not self.config.has_option(
            self._config_holder.section_name, "oem-enc-keypair"
        ):
            self.logger.warning("oem-enc-keypair not found")
            return False
        else:
            self._config_holder.oem_enc_keypair = self.config.get(
                self._config_holder.section_name, "oem-enc-keypair"
            )

        if self.config.has_option(self._config_holder.section_name, "oem-enc-keypwd"):
            self._config_holder.oem_enc_keypwd = self.config.get(
                self._config_holder.section_name, "oem-enc-keypwd"
            )

        if not self.config.has_option(
            self._config_holder.section_name, "asset-filename"
        ):
            self.logger.warning("asset-filename not found")
            return False
        else:
            self._config_holder.asset_filename = self.config.get(
                self._config_holder.section_name, "asset-filename"
            )
        ...
        if not self.config.has_option(self._config_holder.section_name, "pkg-filename"):
            ...
            self._config_holder.pkg_filename = self.config.get(
                self._config_holder.section_name, "pkg-filename"
            )
...
class AssetPackager:
    ...
    def __init__(self, logger, key_cfg):
        self.config = key_cfg
        self.logger = logger
    ...
    def generate_package(self):
        ...
        asset_package = (
            struct.pack("<I", self.PROD_ASSET_PROV_TOKEN)
        ...
        key_tmp = cryptolayer.Common.decrypt_data_with_rsa_keypair(
            self.config.oem_enc_keypair, self.config.oem_enc_keypwd, koem_key
        )
        ...
        encrypted_data_and_tag = cryptolayer.AesCrypto.encrypt_aes_ccm(
            prov_key, nonce, asset_package, asset_data
        )
...
def main(args):
    ...
    the_logger = util_logger.UtilLogger(
        the_argument_parser.log_filename, "OEM asset packaging"
    )
    # get util configuration parameters
    config_parser = AssetPackageConfigParser(
    ...
    asset_provisioner = AssetPackager(the_logger, config_parser.get_config())
    asset_provisioner.generate_package()
    the_logger.info(
        "**** OEM Asset package generation has been completed successfully ****"
    )
