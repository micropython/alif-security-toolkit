...
from isp.otp_check import check_otp_integrity
from isp.trim_decode import trim_decoder
...
def version_decode(isp, message):
    ...
    if len(message) == 136:
        (
            version,
            ALIF_PN,
        ) = struct.unpack("<I16s", bytes(message[2:22]))
