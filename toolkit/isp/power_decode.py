...
def display_cpu_ppu_info(
    ...
    isp_print_color("blue", "+--------+----------+----------+\n")
    isp_print_color(
        "blue" if se_curr == se_prev else "green", "| SE     |0x%08X|" % (se_curr)
    )
    isp_print_color("blue", "0x%08X|\n" % (se_prev))

    isp_print_color(
        "blue" if a32_0_curr == a32_0_prev else "green",
        "| A32_0  |0x%08X|" % (a32_0_curr),
    )
    isp_print_color("blue", "0x%08X|\n" % (a32_0_prev))

    isp_print_color(
        "blue" if a32_1_curr == a32_1_prev else "green",
        "| A32_1  |0x%08X|" % (a32_1_curr),
    )
    isp_print_color("blue", "0x%08X|\n" % (a32_1_prev))

    isp_print_color(
        "blue" if m55_hp_curr == m55_hp_prev else "green",
        "| M55_HP |0x%08X|" % (m55_hp_curr),
    )
    isp_print_color("blue", "0x%08X|\n" % (m55_hp_prev))

    isp_print_color(
        "blue" if m55_he_curr == m55_hp_prev else "green",
...
def display_ppu_info(register_string, ppu_register):
    ...
    if ppu_register == DEVPSTATE_STATE_ON or ppu_register == DEVPSTATE_STATE_OFF_EMU:
        on_state_colour = "green"
    #   on_state_colour = "green" if (ppu_register == DEVPSTATE_STATE_ON ) else "blue"
    isp_print_color(
        on_state_colour,
        "%20s\t0x%08X\t%s\n"
        % (register_string, ppu_register, ppu_status_to_string(ppu_register)),
    )
...
def display_power_info(message):
    ...
    (
        se_curr,
        se_prev,
    ) = struct.unpack("<II", bytes(message[0:8]))
    (
        a32_0_curr,
        a32_0_prev,
    ) = struct.unpack("<II", bytes(message[8:16]))
    (
        a32_1_curr,
        a32_1_prev,
    ) = struct.unpack("<II", bytes(message[16:24]))
    (
        m55_hp_curr,
        m55_hp_prev,
    ) = struct.unpack("<II", bytes(message[24:32]))
    (
        m55_he_curr,
        m55_he_prev,
    ) = struct.unpack("<II", bytes(message[32:40]))
    ...
    (
        ...
        modem_aon_status,
        sse700_aon_status,
    ) = struct.unpack("<IIIIIIIIIIII", bytes(message[52:100]))
