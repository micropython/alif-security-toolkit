...
def display_clock_info(message):
    ...
    isp_print_color(
        "blue", "CLK freq M55-HP %s\n" % (clk_frequency_to_string(frequency))
    )
    # M55-HE frequency
    frequency = (status & 0xFF000000) >> 24
    isp_print_color(
        "blue", "CLK freq M55-HE %s\n" % (clk_frequency_to_string(frequency))
    )
    ...
    for counter in range(len(register_names)):
        ...
        if register_names[counter] == "XO_REG1":
            ...
            ana_reg1.w = value
            value_str = "   osc_rc_32k_freq_cont:{:d}".format(
                ana_reg1.b.osc_rc_32k_freq_cont
            )
            value_str = value_str + "  xtal32k_en:{:d}".format(ana_reg1.b.xtal32k_en)
            value_str = value_str + "  xtal32k_gm_cont:{:d}".format(
                ana_reg1.b.xtal32k_gm_cont
            )
            value_str = value_str + "  xtal32k_cap_cont:{:d}".format(
                ana_reg1.b.xtal32k_cap_cont
            )
            ...
            value_str = value_str + "  osc_76Mrc_cont_bit0:{:d}".format(
                ana_reg2.b.osc_76Mrc_cont_bit0
            )
            value_str = value_str + "  osc_76M_div_cont_fast:{:d}".format(
                ana_reg2.b.osc_76M_div_cont_fast
            )
            value_str = value_str + "  osc_76Mrc_cont:{:d}".format(
                ana_reg2.b.osc_76Mrc_cont
            )
            value_str = value_str + "  osc_76M_div_cont_slow:{:d}".format(
                ana_reg2.b.osc_76M_div_cont_slow
            )
            print(value_str)
