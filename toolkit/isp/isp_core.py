...
from isp.isp_print import isp_print_color, isp_print_message
...
from isp.ospi_decode import ospi_data_decode
from isp.otp import display_otp_info
from isp.key_decode import display_key_storage
...
isp_command_lookup = {
    ...
    ISP_COMMAND_VERIFY_AND_EXECUTE: "COMMAND_VERIFY_AND_EXECUTE",
    ISP_COMMAND_OSPI_WRITE: "COMMAND_OSPI_WRITE",
...
isp_subcommand_lookup = {
    ...
    ISP_GET_KEY_MANAGEMENT: "GET_KEY_MANAGEMENT    ",
    ...
    if len(packet) > ISP_PACKET_HEADER_LENGTH:
        ...
        if isp.getVerbose() is True:
            ...
            if command == ISP_COMMAND_GET or command == ISP_COMMAND_SET:
                ...
                print(" sub command= %4x %s" % (sub_command, sub_command_str), end="")
...
def isp_test_target(baud_rate, isp):
    ...
    when the target did not respond...Int rather returns:
...
def isp_download_data(isp, data):
    ...
    message = isp_build_packet(
        isp, ISP_COMMAND_DOWNLOAD_DATA, ["padding", "padding", bytearray(data)]
    )
    if len(message) == 0:
...
def isp_show_maintenance_mode(isp, maintenance_mode):
    ...
    - disply the maintenance mode status
    """
...
def isp_get_toc_data(isp):
    ...
    while (
        message[ISP_PACKET_COMMAND_FIELD] == ISP_COMMAND_DATA_RESPONSE
        and message[ISP_DATA_START_FIELD] == ISP_DATA_START_MARKER
    ):
...
def isp_get_mram_contents(isp):
    ...
    while (
        message[ISP_PACKET_COMMAND_FIELD] == ISP_COMMAND_DATA_RESPONSE
        and message[ISP_TOC_COMMAND_FIELD] == ISP_TOC_INFO_START_MARKER
    ):
...
def isp_get_seram_metrics(isp):
    ...
    if message[ISP_PACKET_COMMAND_FIELD] == ISP_COMMAND_DATA_RESPONSE:
        ...
        for each_task in range(number_of_tasks):
            stack_size, stack_used, seram_task_name = struct.unpack(
                "<II16s", bytes(element[packet_start : packet_start + 24])
            )
            stack_left = (stack_size * 4) - (stack_used * 4)
            name_str = "".join([chr(e) for e in seram_task_name])

            print_string = " | {:16s} | {:4d} | {:4d} |\n".format(
                str(name_str), stack_size * 4, stack_left
            )
...
def isp_get_seram_trace_buffer(isp):
    ...
    delay = 0
    message = isp_build_packet(
        isp, ISP_COMMAND_GET, [ISP_GET_SERAM_TRACE_BUFFER], delay
    )
...
def isp_get_address(isp, address):
    ...
    delay = 0
    message = isp_build_packet(
        isp, ISP_COMMAND_GET, [ISP_GET_PEEK_ADDRESS, address], delay
    )
    if len(message) != 0:
        cmd = message[ISP_PACKET_COMMAND_FIELD]
...
def isp_get_ecc_key(isp):
    ...
    command = message[ISP_PACKET_COMMAND_FIELD]
    if command != ISP_COMMAND_DATA_RESPONSE:
        print("[ERROR] ECC query error")
        return

    ecc_key = message[2:66]
    print(f"ECC key (HEX): {''.join(f'{n:02X}' for n in ecc_key)}")


def isp_get_key_management_data(isp):
    """
    isp_get_key_management
    """
    message = isp_build_packet(isp, ISP_COMMAND_GET, [ISP_GET_KEY_MANAGEMENT])
    if len(message) == 0:
        return

    command = message[ISP_PACKET_COMMAND_FIELD]
    if command != ISP_COMMAND_DATA_RESPONSE:
        print("[ERROR] Key Management query error")
        return

    #    print(message)
    display_key_storage(message[2::])
...
def isp_set_address(isp, address, data):
    ...
    delay = 0
    message = isp_build_packet(
        isp, ISP_COMMAND_SET, [ISP_SET_POKE_ADDRESS, address, data], delay
    )
    if len(message) != 0:
        cmd = message[ISP_PACKET_COMMAND_FIELD]
...
def isp_set_log_enable(isp, enable):
    ...
    if command == ISP_COMMAND_NAK:
        ...
        isp_print_color("blue", f"[ERROR] {error_string}")
        print(" ")
...
def isp_read_otp(isp, word_offset):
    ...
    if command == ISP_COMMAND_NAK:
        ...
        isp_print_color("blue", f"[ERROR] 0x{word_offset:04x} {error_string}")
        print(" ")
