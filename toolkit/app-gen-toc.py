...
SUPPORTED_FLAGS = ["COMPRESS", "LOAD", "BOOT", "ENCRYPT", "DEFERRED"]
...
def createOemTocPackage(fwsections, outputFile):
    ...
    for sec in fwsections:
        ...
        if sec["signed"] != False:
            ...
            mapf.write(
                f"{hex(mPointer)}\t{hex(CERT_CHAIN_SIZE - CONT_CERT_SIZE)}\t{(CERT_CHAIN_SIZE - CONT_CERT_SIZE)}\tKey1/Key2 Certificates \n"
            )
    ...
    for sec in fwsections:
        ...
        if sec["padlen"] > 0:
            ...
            mapf.write(
                f"{hex(mPointer)}\t{hex(sec['padlen'])}\t{sec['padlen']}\tPadding\n"
            )
            mPointer += sec["padlen"]
    ...
    mapf.write(
        f"{hex(mPointer)}\t{hex(TOC_HEADER_SIZE)}\t{TOC_HEADER_SIZE}\tAPP TOC Header\n"
    )
    ...
    for sec in fwsections:
        ...
        # object_length
        outf.write(
            struct.pack("I", sec["size"])
        )  # do not need to declare the pad as seram doesn't need it
        # object_type
        outf.write(
            struct.pack(
                "I", getObjectType(sec["identifier"].rstrip("\0"), image_switcher)
            )
        )
        # flags
        outf.write(
            struct.pack(
                "I",
                getObjectFlags(sec["flags"]) + getCPUID(sec["cpu_id"], cpuid_switcher),
            )
        )
        ...
        mapf.write(
            f"{hex(mPointer)}\t{hex(TOC_ENTRY_SIZE)}\t{TOC_ENTRY_SIZE}\tAPP TOC entry for {sec['identifier']} obj_address {sec['object_address']}\n"
        )
    ...
    mapf.write(
        f"{hex(mPointer)}\t{hex(TOC_TAIL_SIZE)}\t{TOC_TAIL_SIZE}\tAPP TOC Tail\n"
    )
    ...
    if (mPointer - oemManagedAreaStartAddress) != oemManagedAreaSize:
        print(
            "[ERROR] Something is wrong! Final APP Package size is different than the calculated one..."
        )
    ...
    mapf.write("\nAPP Package Summary:\n")
    mapf.write(
        f" - APP Package total size: {mPointer - oemManagedAreaStartAddress} bytes\n"
    )
    ...
    mapf.write(
        f" - APP TOC size: {(TOC_ENTRY_SIZE * numImages) + TOC_HEADER_SIZE + TOC_TAIL_SIZE} bytes\n"
    )
...
def calcOemManagedArea(fwsections):
    ...
    for sec in fwsections:
        ...
        if "COMPRESS" in sec["flags"]:
            ...
            sec["uncompressedSize"] = sec[
                "size"
            ]  # uncompressed Size will be used by the certificate creation process
            sec["size"] = fsize
            sec["padlen"] = padlen
        ...
        if sec["mramAddress"] == "none" or sec["mramAddress"] == 0:
            ...
            tmp = (
                ...
                compressLabel,
                encryptionLabel,
            )  # we normally don't pad unmanaged images, but just in case this change in the future...
    ...
    oemManagedAreaSize = (
        otocSize
        + sizeManagedImages
        + (numSignedImages * CERT_CHAIN_SIZE)
        + ((numImages - numSignedImages) * CONT_CERT_SIZE)
    )
    ...
    for img in unmanaged:
        ...
        if startAddress < boundary:
            print(
                "[ERROR] There is an overlap between image "
                + img[0]
                + " and previous image!"
            )
    ...
    if boundary > oemManagedAreaStartAddress:
        ...
        print(
            f"Available space is {otocStartAddress - OEM_BASE_ADDRESS} bytes starting at MRAM address {hex(OEM_BASE_ADDRESS)}"
        )
        sys.exit(EXIT_WITH_ERROR)
    else:
        if boundary == 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate APP TOC Package",
    ...
    parser.add_argument(
        "-c",
        "--clean",
        help="Clean build workspace (restore initial state)",
        action="store_true",
    )
